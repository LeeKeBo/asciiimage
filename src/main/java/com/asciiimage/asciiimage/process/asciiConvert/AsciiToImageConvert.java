...
public class AsciiToImageConvert extends ImageConvert<BufferedImage> {
    ...
    @Override
    protected void addTileToOutput(NormalMatrix bestFit, int[] pixels, int row, int column, Dimension tileSize, int imageWidth) {
        if (row < 0 || column < 0)
            throw new IllegalArgumentException("坐标位置无效");
        int[] data = bestFit.getData();
        for (int i = 0; i < tileSize.height; i++) {
            for (int j = 0; j < tileSize.width; j++) {
                int rgbColor = data[i * tileSize.width + j];
                int r = (rgbColor >> 16) & 0XFF;
                int g = (rgbColor >> 8) & 0XFF;
                int b = rgbColor & 0xFF;

                pixels[(i + row) * imageWidth + j + column] = new Color(r,g,b).getRGB();
            }
        }
    }
